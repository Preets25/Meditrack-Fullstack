...
import { User, Stethoscope, Clock, Calendar, Search, MapPin, Star, Phone, ShieldCheck } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredDoctors.map(doctor => (
            <div key={doctor.id} className="bg-white rounded-3xl border border-slate-200 p-6 hover:shadow-xl hover:shadow-indigo-50 transition-all duration-300 group relative overflow-hidden">
              ...
              <button className="w-full py-3 bg-slate-900 text-white rounded-2xl font-bold text-sm hover:bg-indigo-600 transition-colors shadow-lg active:scale-95 duration-150">
                Book Consultation
              </button>
            </div>
          ))}
